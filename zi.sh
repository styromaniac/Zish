...
start_zeronet() {
    ...
    termux-notification --id 1 --title "ZeroNet Running" --content "ZeroNet started with PID \$ZERONET_PID. Tap to manage." --ongoing --button1 "Manage" --button1-action "am start --user 0 -n com.termux/com.termux.app.TermuxActivity"
}
...
EOL
    ...
    log "ZeroNet installation and setup completed successfully."
}

manage_zeronet() {
    while true; do
        ACTION=$(termux-dialog sheet -v "Browse Site,Restart ZeroNet,Exit" -t "ZeroNet Management")
        case $ACTION in
            *"Browse Site"*)
                site_address=$(termux-dialog text -t "Enter ZeroNet Site Address" | jq -r '.text')
                if [ -n "$site_address" ]; then
                    browse_site_files "$site_address"
                fi
                ;;
            *"Restart ZeroNet"*)
                restart_zeronet
                ;;
            *"Exit"*)
                termux-dialog confirm -i "Are you sure you want to exit? This will not stop ZeroNet." -t "Exit Management"
                if [ $? -eq 0 ]; then
                    break
                fi
                ;;
...
browse_site_files() {
    ...
    local site_dir="$ZERONET_DIR/data/$site_address"

    if [ ! -d "$site_dir" ]; then
        termux-toast "Site directory not found: $site_dir"
        return
    fi

    while true; do
        local file_path=$(termux-file-picker -d "$site_dir")
        if [ -z "$file_path" ]; then
            break
        elif [ -f "$file_path" ]; then
            ACTION=$(termux-dialog sheet -v "Open,Edit,Delete,Back" -t "File: $(basename "$file_path")")
            case $ACTION in
                *"Open"*)
                ...
                *"Edit"*)
                    ...
                    ;;
                *"Delete"*)
                    termux-dialog confirm -i "Are you sure you want to delete $(basename "$file_path")?" -t "Confirm Delete"
                    if [ $? -eq 0 ]; then
                        rm "$file_path"
                        termux-toast "File deleted: $(basename "$file_path")"
                    else
                        termux-toast "Delete cancelled"
                    fi
                    ;;
                *"Back"*)
                    continue
...
restart_zeronet() {
    ZERONET_PID=$(pgrep -f "python3.*zeronet.py")
    if [ -n "$ZERONET_PID" ]; then
        kill $ZERONET_PID
        sleep 5  # Wait for the process to terminate
    fi
    cd $ZERONET_DIR
    ...
    python3 zeronet.py --config_file $ZERONET_DIR/zeronet.conf &
    NEW_ZERONET_PID=$!
    termux-notification --id 1 --title "ZeroNet Restarted" --content "New PID: $NEW_ZERONET_PID. Tap to manage." --ongoing --button1 "Manage" --button1-action "am start --user 0 -n com.termux/com.termux.app.TermuxActivity"
}
